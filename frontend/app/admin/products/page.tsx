...
export default function AdminProductsPage() {
  ...
  const loadProducts = () => api.get('/products/admin/all').then((res) => setProducts(res.data.products));
  const loadCategories = () => api.get('/categories/admin/all').then((res) => setCategories(res.data));
  ...
  return (
    <AdminLayout>
      <h1 className="text-2xl font-serif font-semibold mb-8">Products</h1>
          ...
          <div key={p._id} className="card p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                ₹{p.basePrice} · {p.category?.name} · {p.variants?.length || 0} variant(s) · Stock: {p.totalStock}
                {!p.isActive && <span className="text-red-500"> · Inactive</span>}
              </p>
            </div>
            <div className="flex gap-3 text-sm">
              <button onClick={() => handleEdit(p)} className="text-brand-500">Edit</button>
